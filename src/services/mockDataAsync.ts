...
function getData(): Promise<TodosType[]> {
...
function getTodosForDay(date: Date): Promise<TodosType[]> {
  return new Promise((resolve) => {
    setTimeout(() => {
      const result = mockdata.filter((todo) => todo.date.toLocaleDateString() == date.toLocaleDateString());
      resolve(result);
    }, 1000);
  });
}

function getTodosForWeek(dates: Date[]): Promise<TodosType[]> {
  return new Promise((resolve) => {
    setTimeout(() => {
...
function getSingleTodo(id: string) : Promise<TodosType> {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const todo = mockdata.find((todo) => todo.id === id);
      if (!todo) reject(new Error("Todo not found"));
      else resolve(todo);
    }, 1000);
  });
}
...
export { getData, getTodosForDay, getTodosForWeek, getSingleTodo};
